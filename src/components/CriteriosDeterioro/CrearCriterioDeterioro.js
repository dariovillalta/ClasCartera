...
export default class CrearCriterioDeterioro extends React.Component {
    ...
    iniciarCriterioDeterioro() {
        ...
        transaction.begin(err => {
            ...
            request.query("select * from CriterioDeterioro where categoriaClasPadre = "+idCategoria+" and tipoDeCredito = "+idTipoCredito+" and tipoGarantia = '"+tipoGarantia+"'", (err, result) => {
                if (err) {
                    if (!rolledBack) {
                        console.log(err);
    ...
    guardarCriterioDeterioro(criterioDeterioroMismaCategoria) {
        ...
        let tipoCredito = this.props.tiposDeCredito[$("#tipoCreditoID").val()].ID;
                                        ...
                                        transaction.begin(err => {
                                            ...
                                            request.query("insert into CriterioDeterioro (categoriaClasPadre, nombreClasPadre, categoria, tipoDeCredito, campoTipoGarantia, tipoGarantia, inicioMora, finMora, estimacionDeterioro) values ("+categoriaClasificacion+",'"+nombreClasPadre+"','"+categoria+"',"+tipoCredito+","+campoTipoGarantia+",'"+tipoGarantia+"',"+inicioMora+","+finMora+","+porcentajeEstimacionDeterioro+")", (err, result) => {
                                                if (err) {
                                                    ...
                                                    transaction.commit(err => {
                                                        this.showSuccesMessage("Exito", "Criterio de Deterioro creado con éxito.");
                                                    });
                                                }
                                            });
                                        }); // fin transaction
                            ...
                            <div className={"card-body"}>
                                <div className={"d-inline-block text-center"} style={{width: "100%"}}>
                                    ...
                                    <select id="tipoCreditoID" className={"form-control form-control-lg"}>
                                        {this.props.tiposDeCredito.map((tipoDeCredito, i) => {
                        ...
                        <div className={"card"}>
                            <div className={"card-body"}>
                                <div className={"d-inline-block text-center"} style={{width: "100%"}}>
                                    ...
                                    <select id={"campo"} className={"form-control"}>
